...
---- GAZZSPY - Modified from SimpleSpy V3 Mobile ----
...
-- ══════════════════════════════════════════════════
-- GAZZSPY: Màu phân biệt RemoteEvent vs RemoteFunction
-- ══════════════════════════════════════════════════
local REMOTE_EVENT_COLOR    = Color3.fromRGB(50, 220, 120)   -- Xanh lá  → RemoteEvent  (:FireServer)
local REMOTE_FUNCTION_COLOR = Color3.fromRGB(100, 140, 255)  -- Xanh dương → RemoteFunction (:InvokeServer)
...
-- ══════════════════════════════════════════════════
-- GAZZSPY: Anti Duplicate Spy - 100% CHÍNH XÁC
-- Nguyên tắc: So sánh hash sâu toàn bộ args (kể cả table lồng nhau, số thực)
-- KHÔNG dùng cooldown thời gian → chỉ bỏ khi args byte-for-byte giống nhau
-- 50% HP ≠ 100% HP → hash khác → vẫn spy 100%
-- ══════════════════════════════════════════════════
local antiDupEnabled = true
local recentRemoteKeys = {}  -- [remote] = lastArgsHash (string)

-- Serialize sâu toàn bộ args thành chuỗi hash duy nhất
-- Dùng đệ quy để xử lý table lồng nhau, giữ nguyên độ chính xác số thực
local function deepSerialize(v, depth, visited)
	depth = depth or 0
	visited = visited or {}
	...
	elseif t == "number" then
		-- Dùng %.10g để giữ đủ độ chính xác, phân biệt được 0.5 vs 1.0
		return string.format("%.10g", v)
	elseif t == "string" then
		-- Escape toàn bộ string để tránh nhầm lẫn separator
		return '"' .. v:gsub("\\", "\\\\"):gsub('"', '\\"') .. '"'
	elseif t == "Instance" then
		-- Dùng full path để phân biệt instance khác nhau có cùng tên
		local ok, path = pcall(function() return v:GetFullName() end)
		return "inst[" .. (ok and path or tostring(v)) .. "]"
	elseif t == "Vector3" then
		return string.format("V3(%.10g,%.10g,%.10g)", v.X, v.Y, v.Z)
	elseif t == "Vector2" then
		return string.format("V2(%.10g,%.10g)", v.X, v.Y)
	elseif t == "CFrame" then
		local x,y,z = v:ToEulerAnglesXYZ()
		return string.format("CF(%.10g,%.10g,%.10g,%.10g,%.10g,%.10g)", v.X,v.Y,v.Z,x,y,z)
	elseif t == "Color3" then
		return string.format("C3(%.10g,%.10g,%.10g)", v.R,v.G,v.B)
	elseif t == "UDim2" then
		return string.format("UD2(%.10g,%.10g,%.10g,%.10g)", v.X.Scale,v.X.Offset,v.Y.Scale,v.Y.Offset)
	elseif t == "UDim" then
		return string.format("UD(%.10g,%.10g)", v.Scale,v.Offset)
	elseif t == "EnumItem" then
		return "E[" .. tostring(v) .. "]"
	elseif t == "table" then
		-- Tránh vòng lặp vô tận với table tự tham chiếu
		if visited[v] then return "<CIRCULAR>" end
		visited[v] = true
		local parts = {}
		-- Sort keys để đảm bảo thứ tự nhất quán dù Lua iterate không đảm bảo thứ tự
		local keys = {}
		...
		for _, k in ipairs(keys) do
			table.insert(parts, deepSerialize(k, depth+1, visited) .. "=" .. deepSerialize(v[k], depth+1, visited))
		end
		visited[v] = nil
		return "{" .. table.concat(parts, ";") .. "}"
	elseif t == "BrickColor" then
		return "BC[" .. tostring(v.Number) .. "]"
	elseif t == "NumberRange" then
		return string.format("NR(%.10g,%.10g)", v.Min, v.Max)
	else
...
--- Tạo hash từ toàn bộ args array
local function makeArgsHash(args)
	local ok, result = pcall(function()
		local parts = {}
		for i = 1, #args do
			parts[i] = deepSerialize(args[i])
		end
		-- Thêm số lượng args vào hash để phân biệt (1 arg) vs (1 arg, nil)
		return "#" .. #args .. "|" .. table.concat(parts, "||")
	end)
	if ok then return result end
	-- Nếu serialize thất bại (args cực kỳ phức tạp), không bỏ qua để an toàn
	return nil
end

--- Kiểm tra có phải spy trùng lặp không - CHÍNH XÁC 100%
--- Trả về true nếu TRÙNG HOÀN TOÀN (bỏ qua), false nếu KHÁC (spy bình thường)
local function isDuplicate(remote, args)
	if not antiDupEnabled then return false end
	local newHash = makeArgsHash(args)
	-- Nếu không serialize được → không bỏ qua, cứ spy để an toàn
	if newHash == nil then return false end
	local lastHash = recentRemoteKeys[remote]
	-- Chỉ trùng khi hash byte-for-byte giống nhau
	if lastHash == newHash then
		return true  -- TRÙNG HOÀN TOÀN → bỏ qua
	end
	-- Hash khác → args khác → cập nhật và spy bình thường
	recentRemoteKeys[remote] = newHash
	return false
end
...
-- Tạo nội dung codebox đầy đủ = GenScript + Function Info tự động chi tiết
local function buildFullCodeContent(log)
	local content = log.GenScript or "-- Đang generate..."
	-- Function Info tự động thêm vào cuối nếu có, không cần bấm nút
	if log.Function and log.Function ~= "" then
		content = content .. "\n\n" .. tostring(log.Function)
	end
	return content
end
...
function eventSelect(frame)
	...
	if selected and selected.Log then
		...
		-- Hiển thị GenScript + Function Info tự động ngay khi click, không cần bấm nút
		codebox:setRaw(buildFullCodeContent(selected))
	end
...
-- ══════════════════════════════════════════════════
-- GAZZSPY: newRemote với màu phân biệt
-- RemoteEvent  → Xanh lá  (REMOTE_EVENT_COLOR)
-- RemoteFunction → Xanh dương (REMOTE_FUNCTION_COLOR)
-- ══════════════════════════════════════════════════
function newRemote(remoteType, name, args, remote, function_info, blocked, src, returnValue)
	local remoteFrame = RemoteTemplate:Clone()
	remoteFrame.Text.Text = string.sub(name, 1, 50)

	-- Gán màu theo loại remote
	if remoteType == "event" then
		remoteFrame.ColorBar.BackgroundColor3 = REMOTE_EVENT_COLOR      -- Xanh lá = FireServer
	else
		remoteFrame.ColorBar.BackgroundColor3 = REMOTE_FUNCTION_COLOR   -- Xanh dương = InvokeServer
	end
	...
	schedule(function()
		...
		-- Nếu log này đang được chọn, tự refresh codebox với nội dung đầy đủ
		if selected and selected.Log == log.Log then
			codebox:setRaw(buildFullCodeContent(log))
		end
	end)
...
function u2s(u)
	...
	elseif typeof(u) == "Region3" then
		...
		return "Region3.new(" .. u2s(center - size / 2) .. ", " .. u2s(center + size / 2) .. ")"
...
-- ══════════════════════════════════════════════════
-- GAZZSPY: remoteHandler tích hợp anti-duplicate
-- ══════════════════════════════════════════════════
function remoteHandler(hookfunction, methodName, remote, args, funcInfo, calling, returnValue)
	...
	if validInstance and validClass then
		...
		if isDuplicate(remote, args) then return end
		...
		if func and islclosure(func) then
			...
			-- Build Function Info chi tiết, rõ ràng, dễ đọc
			pcall(function()
				local info = funcInfo
				local lines = {}
				table.insert(lines, "-- ╔══════════════════════════════════════════╗")
				table.insert(lines, "-- ║         GAZZSPY - FUNCTION INFO          ║")
				table.insert(lines, "-- ╚══════════════════════════════════════════╝")
				table.insert(lines, "")
				-- Tên hàm
				local funcName = (info and info.name and info.name ~= "") and info.name or "(anonymous)"
				table.insert(lines, "-- Tên hàm     : " .. funcName)
				-- Source file
				local srcStr = "(không xác định)"
				if src then
					pcall(function() srcStr = src:GetFullName() end)
				elseif info and info.source then
					srcStr = tostring(info.source)
				end
				table.insert(lines, "-- Script       : " .. srcStr)
				-- Dòng định nghĩa hàm
				if info and info.linedefined and info.linedefined > 0 then
					table.insert(lines, "-- Defined line : " .. tostring(info.linedefined))
				end
				-- Dòng hiện tại đang chạy
				if info and info.currentline and info.currentline > 0 then
					table.insert(lines, "-- Current line : " .. tostring(info.currentline))
				end
				-- Số params
				if info and info.nparams ~= nil then
					table.insert(lines, "-- Params       : " .. tostring(info.nparams))
				end
				-- Có vararg không
				if info and info.is_vararg ~= nil then
					table.insert(lines, "-- Vararg       : " .. (info.is_vararg == 1 and "true" or "false"))
				end
				table.insert(lines, "")
				-- Constants (giá trị literal dùng trong hàm)
				local consts = {}
				pcall(function() consts = debug.getconstants(func) end)
				if #consts > 0 then
					table.insert(lines, "-- Constants (" .. #consts .. "):")
					for i, c in ipairs(consts) do
						local ct = type(c)
						local cv
						if ct == "string" then cv = '"' .. tostring(c):sub(1, 80) .. '"'
						elseif ct == "number" then cv = tostring(c)
						elseif ct == "boolean" then cv = tostring(c)
						elseif c == nil then cv = "nil"
						else cv = "(" .. ct .. ")" end
						table.insert(lines, "--   [" .. i .. "] " .. cv)
					end
					table.insert(lines, "")
				end
				-- Upvalues (biến bên ngoài hàm đang capture)
				local upvals = {}
				pcall(function()
				...
				if upvals and #upvals > 0 then
					table.insert(lines, "-- Upvalues (" .. #upvals .. "):")
					for i, uv in ipairs(upvals) do
						local uvt = typeof(uv)
						local uvv
						if uvt == "string" then uvv = '"' .. tostring(uv):sub(1, 60) .. '"'
						elseif uvt == "number" or uvt == "boolean" then uvv = tostring(uv)
						elseif uv == nil then uvv = "nil"
						elseif uvt == "Instance" then pcall(function() uvv = uv:GetFullName() end); uvv = uvv or tostring(uv)
						else uvv = "(" .. uvt .. ")" end
						table.insert(lines, "--   [" .. i .. "] " .. uvv)
					end
					table.insert(lines, "")
				end
				-- Proto names (sub-functions bên trong)
				local protos = {}
				pcall(function()
					if debug.getprotos then protos = debug.getprotos(func) end
				end)
				if protos and #protos > 0 then
					table.insert(lines, "-- Sub-functions (" .. #protos .. "):")
					for i, proto in ipairs(protos) do
						local pinfo = {}
						pcall(function() pinfo = debug.getinfo(proto) end)
						local pname = (pinfo and pinfo.name and pinfo.name ~= "") and pinfo.name or "(anonymous)"
						local pline = (pinfo and pinfo.linedefined and pinfo.linedefined > 0) and (" @ line " .. pinfo.linedefined) or ""
						table.insert(lines, "--   [" .. i .. "] " .. pname .. pline)
					end
					table.insert(lines, "")
				end
				functionInfoStr = table.concat(lines, "\n")
			end)
		end
...
local newnamecall = newcclosure(function(remote, ...)
	if typeof(remote) == "Instance" then
		...
		elseif validInstance and (methodName == "FireServer" or methodName == "fireServer" or methodName == "InvokeServer" or methodName == "invokeServer") and (blocklist[remote] or blocklist[remoteName]) then return nil
...
function toggleSpy()
	if not toggle then
		if hookmetamethod then
			...
			if not original then warn("GazzSpy: namecall method not found!"); onToggleButtonClick(); return end
...
-- ══════════════════════════════════════════════════
-- GAZZSPY: SaveAll - Lưu tất cả logs vào thư mục GazzSpy
-- ══════════════════════════════════════════════════
local function saveAllLogs()
	if #logs == 0 then
		TextLabel.Text = "Không có log nào để lưu!"
		return
	end
	local folderName = "GazzSpy"
	if not isfolder(folderName) then
		makefolder(folderName)
	end
	local timestamp = tostring(math.floor(tick()))
	local allContent = "-- GazzSpy SaveAll | " .. timestamp .. " | Game: " .. tostring(game.PlaceId) .. "\n"
		.. "-- Tổng log: " .. tostring(#logs) .. "\n"
		.. "-- Màu sắc: Xanh lá = RemoteEvent (:FireServer) | Xanh dương = RemoteFunction (:InvokeServer)\n"
		.. string.rep("=", 60) .. "\n\n"

	for i, log in ipairs(logs) do
		local logType = log.Type == "event" and "[RemoteEvent]" or "[RemoteFunction]"
		allContent = allContent
			.. "-- (" .. i .. ") " .. logType .. " " .. tostring(log.Name) .. "\n"
			.. tostring(log.GenScript) .. "\n"
			.. string.rep("-", 40) .. "\n\n"
	end

	local fileName = folderName .. "/GazzSpy_" .. timestamp .. ".lua"
	writefile(fileName, allContent)
	TextLabel.Text = "Đã lưu " .. #logs .. " log vào " .. fileName
end
...
newButton("Copy Remote", function()
	return "Nhấn để copy đường dẫn remote"
end, function()
	if selected then
		setclipboard(v2s(selected.Remote.remote))
		TextLabel.Text = "Đã copy remote!"
	end
end)

-- Run Code
-- Tạm blacklist remote đang chọn trong lúc run → tránh bị spy loop lại
newButton("Run Code", function()
	return "Nhấn để chạy code trong codebox"
end, function()
	TextLabel.Text = "Đang chạy..."
	local runRemote = selected and selected.Remote and selected.Remote.remote or nil
	local runRemoteName = selected and selected.Name or nil
	-- Tạm thời tắt spy remote này
	if runRemote then blacklist[runRemote] = true end
	if runRemoteName then blacklist[runRemoteName] = true end
	local succeeded, errMsg = pcall(function() return loadstring(codebox:getString())() end)
	-- Bỏ chặn sau 0.15s (đủ để call đã qua hook)
	task.delay(0.15, function()
		if runRemote then blacklist[runRemote] = nil end
		if runRemoteName then blacklist[runRemoteName] = nil end
	end)
	if succeeded then TextLabel.Text = "Chay thanh cong!"
	else TextLabel.Text = "Loi: " .. tostring(errMsg):sub(1, 60) end
end)

-- Get Script
newButton("Get Script", function()
	return "Nhấn để copy script gọi remote vào clipboard"
end, function()
	if selected then
...
newButton("Clr Logs", function()
	return "Nhấn để xóa tất cả logs"
end, function()
	...
	codebox:setRaw("")
	selected = nil
	TextLabel.Text = "Đã xóa logs!"
end)

-- Exclude by Instance
newButton("Exclude (i)", function()
	return "Nhấn để loại trừ remote này (theo instance)"
end, function()
	if selected then
		blacklist[selected.Remote.remote] = true
		TextLabel.Text = "Đã loại trừ!"
	end
end)

-- Exclude by Name
newButton("Exclude (n)", function()
	return "Nhấn để loại trừ tất cả remote có cùng tên"
end, function()
	if selected then
		blacklist[selected.Name] = true
		TextLabel.Text = "Đã loại trừ theo tên!"
	end
end)

-- Clear Blacklist
newButton("Clr Blacklist", function()
	return "Nhấn để xóa blacklist"
end, function()
	blacklist = {}
	TextLabel.Text = "Đã xóa blacklist!"
end)

-- Block by Instance
newButton("Block (i)", function()
	return "Nhấn để chặn remote này không gửi lên server"
end, function()
	if selected then
		if selected.Remote.remote then
			blocklist[selected.Remote.remote] = true
			TextLabel.Text = "Đã chặn!"
		else TextLabel.Text = "Lỗi! Thử Block (n)" end
	end
end)

-- Block by Name
newButton("Block (n)", function()
	return "Nhấn để chặn tất cả remote có cùng tên"
end, function()
	if selected then
		blocklist[selected.Name] = true
		TextLabel.Text = "Đã chặn theo tên!"
	end
end)

-- Clear Blocklist
newButton("Clr Blocklist", function()
	return "Nhấn để xóa blocklist"
end, function()
	blocklist = {}
	TextLabel.Text = "Đã xóa blocklist!"
end)

-- ══════════════════════════════════════════════════
-- GAZZSPY: Anti Duplicate Toggle
-- Thông minh: 50% HP vs 100% HP = args KHÁC → KHÔNG trùng → vẫn spy
-- Chỉ bỏ khi args HOÀN TOÀN giống nhau trong thời gian cực ngắn (0.05s)
-- ══════════════════════════════════════════════════
newButton("Anti Dup", function()
	return string.format(
		"[%s] Chống Spy Trùng Lặp Thông Minh\n"
		.. "• Chỉ bỏ qua khi args HOÀN TOÀN giống nhau\n"
		.. "• 50%% HP vs 100%% HP = args KHÁC → vẫn spy bình thường",
		antiDupEnabled and "BẬT" or "TẮT"
	)
end, function()
	antiDupEnabled = not antiDupEnabled
	recentRemoteKeys = {}
	TextLabel.Text = string.format("Chống trùng lặp: %s", antiDupEnabled and "BẬT" or "TẮT")
end)

-- Autoblock
newButton("Autoblock", function()
	return string.format("[%s] [BETA] Tự động phát hiện và loại remote spam", autoblock and "BẬT" or "TẮT")
end, function()
	autoblock = not autoblock
	TextLabel.Text = string.format("Autoblock: %s", autoblock and "BẬT" or "TẮT")
	history = {}
	excluding = {}
end)
...
newButton("SaveAll", function()
	return "Nhấn để lưu TẤT CẢ logs vào thư mục GazzSpy\nFile được lưu theo timestamp"
end, function()
	saveAllLogs()
end)
