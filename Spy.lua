...
-- ══════════════════════════════════════════════════
-- GAZZSPY: SaveAll - Mỗi remote thành 1 file riêng trong thư mục GazzSpy
-- Ví dụ: TakeDamage_1.lua, TakeDamage_2.lua, FireBullet_1.lua ...
-- Dễ tìm, dễ làm script từng cái riêng
-- ══════════════════════════════════════════════════
local function saveAllLogs()
	if #logs == 0 then
		TextLabel.Text = "Khong co log nao de luu!"
		return
	end
	...
	-- Đếm số lần xuất hiện của từng tên remote để đánh số _1, _2, _3 ...
	-- Ví dụ: TakeDamage_1.lua (50% HP), TakeDamage_2.lua (100% HP)
	local nameCount = {}   -- [remoteName] = số thứ tự hiện tại
	local savedCount = 0

	-- logs được thêm mới nhất lên đầu (layoutOrder giảm dần)
	-- Đảo lại để lưu theo thứ tự thời gian xảy ra (cũ → mới)
	local ordered = {}
	for i = #logs, 1, -1 do
		table.insert(ordered, logs[i])
	end

	for _, log in ipairs(ordered) do
		local remoteName = tostring(log.Name)

		-- Làm sạch tên file: chỉ giữ chữ, số, gạch dưới (tránh lỗi filesystem)
		local safeName = remoteName:gsub("[^%w_]", "_")
		if safeName == "" then safeName = "Unknown" end

		-- Đánh số thứ tự theo tên
		nameCount[safeName] = (nameCount[safeName] or 0) + 1
		local fileIndex = nameCount[safeName]

		-- Loại remote
		local remoteType = log.Type == "event" and "RemoteEvent" or "RemoteFunction"

		-- Nội dung file
		local content = "-- GazzSpy | " .. remoteType .. " | " .. remoteName .. " #" .. fileIndex .. "\n"
			.. "-- Game: " .. tostring(game.PlaceId) .. "\n"
			.. string.rep("-", 50) .. "\n\n"
			.. tostring(log.GenScript or "-- (chua generate xong)")
		...
		-- Tên file: RemoteName_1.lua, RemoteName_2.lua, ...
		local fileName = folderName .. "/" .. safeName .. "_" .. fileIndex .. ".lua"
		pcall(writefile, fileName, content)
		savedCount = savedCount + 1
	end

	TextLabel.Text = "Da luu " .. savedCount .. " file vao thu muc GazzSpy/"
end
